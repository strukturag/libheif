...
static struct heif_error error_Ok = {heif_error_Ok, heif_suberror_Unspecified,
                                     kSuccess};
static struct heif_error error_unsupported_parameter = {
    heif_error_Usage_error, heif_suberror_Unsupported_parameter,
    "Unsupported encoder parameter"};
static struct heif_error error_unsupported_plugin_version = {
    heif_error_Usage_error, heif_suberror_Unsupported_plugin_version,
...
heif_error heif_context_read_from_file(heif_context* ctx,
                                       const char* filename,
...
heif_error heif_context_read_from_memory(heif_context* ctx,
                                         const void* mem,
                                         size_t size,
                                         const struct heif_reading_options*) {
...
heif_error heif_context_read_from_reader(
    ...
    const struct heif_reading_options*) {
  auto reader =
      std::make_shared<StreamReader_CApi>(reader_func_table, userdata);

  Error err = ctx->context->read(reader);
  return err.error_struct(ctx->context.get());
}

// TODO: heif_error heif_context_read_from_file_descriptor(heif_context*, int
// fd);
...
heif_error heif_context_get_primary_image_handle(heif_context* ctx,
                                                 heif_image_handle** img) {
  if (!img) {
    Error err(heif_error_Usage_error, heif_suberror_Null_pointer_argument);
    return err.error_struct(ctx->context.get());
  }

  std::shared_ptr<HeifContext::Image> primary_image =
      ctx->context->get_primary_image();
...
struct heif_error heif_context_get_primary_image_ID(struct heif_context* ctx,
  ...
  std::shared_ptr<HeifContext::Image> primary =
      ctx->context->get_primary_image();
  if (!primary) {
...
int heif_context_is_top_level_image_ID(struct heif_context* ctx,
                                       heif_item_id id) {
  const std::vector<std::shared_ptr<HeifContext::Image>> images =
      ctx->context->get_top_level_images();
...
int heif_context_get_list_of_top_level_image_IDs(struct heif_context* ctx,
  ...
  const std::vector<std::shared_ptr<HeifContext::Image>> imgs =
      ctx->context->get_top_level_images();
...
struct heif_error heif_decode_image(
  ...
  Error err = in_handle->image->decode_image(img, colorspace, chroma, options);
...
struct heif_error heif_image_create(int width,
  ...
  *image = img;
  ...
  struct heif_error err = {heif_error_Ok, heif_suberror_Unspecified,
                           Error::kSuccess};
  return err;
}
...
int heif_image_get_bits_per_pixel(const struct heif_image* img,
                                  enum heif_channel channel) {
  return img->image->get_bits_per_pixel(channel);
}

int heif_image_has_channel(const struct heif_image* img,
                           enum heif_channel channel) {
...
struct heif_error heif_image_set_raw_color_profile(
  ...
  if (strlen(color_profile_type_fourcc) != 4) {
    heif_error err = {heif_error_Usage_error, heif_suberror_Unspecified,
  ...
  data.insert(data.end(), (const uint8_t*)profile_data,
              (const uint8_t*)profile_data + profile_size);

  auto color_profile =
      std::make_shared<color_profile_raw>(color_profile_type, data);
...
int heif_image_handle_get_number_of_metadata_blocks(
  ...
  for (const auto& metadata : metadata_list) {
    if (type_filter == nullptr || metadata->item_type == type_filter) {
      cnt++;
    }
  }
...
struct heif_error heif_image_handle_get_metadata(
  ...
  for (auto metadata : metadata_list) {
    if (metadata->item_id == metadata_id) {
      memcpy(out_data, metadata->m_data.data(), metadata->m_data.size());
...
struct heif_error heif_image_handle_get_nclx_color_profile(
  ...
  if (nclx_profile) {
    ...
    *out_data = (struct heif_color_profile_nclx*)malloc(
        sizeof(struct heif_color_profile_nclx));

    struct heif_color_profile_nclx* nclx = *out_data;

    nclx->version = 1;
    nclx->color_primaries =
        (enum heif_color_primaries)nclx_profile->get_colour_primaries();
    nclx->transfer_characteristics =
        (enum heif_transfer_characteristics)
            nclx_profile->get_transfer_characteristics();
    nclx->matrix_coefficients =
        (enum heif_matrix_coefficients)nclx_profile->get_matrix_coefficients();
    ...
    Error err(heif_error_Usage_error, heif_suberror_Unspecified);
    return err.error_struct(handle->image.get());
  }
}
...
struct heif_error heif_image_handle_get_raw_color_profile(
  ...
  auto raw_profile = std::dynamic_pointer_cast<color_profile_raw>(profile);
  if (raw_profile) {
    memcpy(out_data, raw_profile->get_data().data(),
...
// DEPRECATED
struct heif_error heif_register_decoder(
    heif_context* heif,
    const heif_decoder_plugin* decoder_plugin) {
...
struct heif_error heif_register_encoder_plugin(
    const heif_encoder_plugin* encoder_plugin) {
  ...
  if (descriptors.size() > 0) {
    if (context == nullptr) {
      *encoder = new struct heif_encoder(nullptr, descriptors[0]->plugin);
      ...
      // DEPRECATED. We do not need the context anywhere.
      *encoder =
          new struct heif_encoder(context->context, descriptors[0]->plugin);
    }
    ...
    struct heif_error err = {
        heif_error_Unsupported_filetype,  // TODO: is this the right error code?
...
struct heif_error heif_encoder_set_lossless(struct heif_encoder* encoder,
                                            int enable) {
  if (!encoder) {
    return Error(heif_error_Usage_error, heif_suberror_Null_pointer_argument)
        .error_struct(nullptr);
...
struct heif_error heif_encoder_set_logging_level(struct heif_encoder* encoder,
                                                 int level) {
  if (!encoder) {
    return Error(heif_error_Usage_error, heif_suberror_Null_pointer_argument)
        .error_struct(nullptr);
  ...
  if (encoder->plugin->set_parameter_logging_level) {
    return encoder->plugin->set_parameter_logging_level(encoder->encoder,
                                                        level);
  }
...
struct heif_error heif_encoder_set_parameter_integer(
    struct heif_encoder* encoder,
    const char* parameter_name,
    int value) {
  return encoder->plugin->set_parameter_integer(encoder->encoder,
                                                parameter_name, value);
}

struct heif_error heif_encoder_get_parameter_integer(
    struct heif_encoder* encoder,
    const char* parameter_name,
    int* value_ptr) {
  return encoder->plugin->get_parameter_integer(encoder->encoder,
                                                parameter_name, value_ptr);
}
...
struct heif_error heif_encoder_parameter_integer_valid_range(
    struct heif_encoder* encoder,
    const char* parameter_name,
    int* have_minimum_maximum,
    int* minimum,
...
struct heif_error heif_encoder_set_parameter_boolean(
    struct heif_encoder* encoder,
    const char* parameter_name,
    int value) {
  return encoder->plugin->set_parameter_boolean(encoder->encoder,
                                                parameter_name, value);
}

struct heif_error heif_encoder_get_parameter_boolean(
    struct heif_encoder* encoder,
    const char* parameter_name,
    int* value_ptr) {
  return encoder->plugin->get_parameter_boolean(encoder->encoder,
                                                parameter_name, value_ptr);
}

struct heif_error heif_encoder_set_parameter_string(
    struct heif_encoder* encoder,
    const char* parameter_name,
    const char* value) {
  return encoder->plugin->set_parameter_string(encoder->encoder, parameter_name,
                                               value);
}

struct heif_error heif_encoder_get_parameter_string(
    struct heif_encoder* encoder,
    const char* parameter_name,
    char* value_ptr,
    int value_size) {
...
struct heif_error heif_encoder_parameter_string_valid_values(
  ...
  for (const struct heif_encoder_parameter* const* params =
       ...
       *params; params++) {
    if (strcmp((*params)->name, parameter_name) == 0) {
      return heif_encoder_parameter_get_valid_string_values(*params,
                                                            out_stringarray);
    }
  }
  ...
  for (const struct heif_encoder_parameter* const* params =
    ...
    if (strcmp((*params)->name, parameter_name) == 0) {
      ...
      case heif_encoder_parameter_type_boolean:
        return heif_encoder_set_parameter_boolean(encoder, parameter_name,
                                                  parse_boolean(value));

      case heif_encoder_parameter_type_string:
        return heif_encoder_set_parameter_string(encoder, parameter_name,
                                                 value);
        break;
      }
    ...
    if (strcmp((*params)->name, parameter_name) == 0) {
      ...
      case heif_encoder_parameter_type_integer: {
        ...
        struct heif_error error =
            heif_encoder_get_parameter_integer(encoder, parameter_name, &value);
        if (error.code) {
          return error;
        } else {
      ...
      case heif_encoder_parameter_type_boolean: {
        int value;
        struct heif_error error =
            heif_encoder_get_parameter_boolean(encoder, parameter_name, &value);
        if (error.code) {
          return error;
        } else {
          snprintf(value_ptr, value_size, "%d", value);
      ...
      case heif_encoder_parameter_type_string: {
        struct heif_error error = heif_encoder_get_parameter_string(
            encoder, parameter_name, value_ptr, value_size);
        if (error.code) {
          return error;
        }
      } break;
...
struct heif_error heif_context_encode_image(
  ...
  error = ctx->context->encode_image(input_image->image, encoder, options,
                                     heif_image_input_class_normal, image);
  if (error != Error::Ok) {
...
struct heif_error heif_context_assign_thumbnail(
  ...
  Error error = ctx->context->assign_thumbnail(thumbnail_image->image,
                                               master_image->image);
  return error.error_struct(ctx->context.get());
}
...
struct heif_error heif_context_encode_thumbnail(
  ...
  Error error = ctx->context->encode_thumbnail(image->image, encoder, options,
                                               bbox_size, thumbnail_image);
  if (error != Error::Ok) {
    return error.error_struct(ctx->context.get());
  }
...
struct heif_error heif_context_add_exif_metadata(
    ...
    int size) {
  Error error =
      ctx->context->add_exif_metadata(image_handle->image, data, size);
  if (error != Error::Ok) {
    return error.error_struct(ctx->context.get());
  } else {
    return error_Ok;
