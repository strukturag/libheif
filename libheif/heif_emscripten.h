...
static void strided_copy(void* dest,
                         const void* src,
                         int width,
                         int height,
                         int stride) {
...
static emscripten::val heif_js_context_get_list_of_top_level_image_IDs(
  ...
  int received =
      heif_context_get_list_of_top_level_image_IDs(context, ids, count);
  if (!received) {
    free(ids);
    return result;
  }
...
static emscripten::val heif_js_decode_image(struct heif_image_handle* handle,
  ...
  struct heif_error err =
      heif_decode_image(handle, &image, colorspace, chroma, nullptr);
  if (err.code != heif_error_Ok) {
    return emscripten::val(err);
  }
  ...
  case heif_colorspace_YCbCr: {
    ...
    strided_copy(dest + (width * height), plane_u, width / 2, height / 2,
                 stride_u);
    strided_copy(dest + (width * height) + (width * height / 4), plane_v,
                 width / 2, height / 2, stride_v);
  } break;
  case heif_colorspace_RGB: {
    assert(heif_image_get_chroma_format(image) ==
           heif_chroma_interleaved_24bit);
    int stride_rgb;
    const uint8_t* plane_rgb = heif_image_get_plane_readonly(
  ...
  case heif_colorspace_monochrome: {
    ...
    int stride_grey;
    const uint8_t* plane_grey =
        heif_image_get_plane_readonly(image, heif_channel_Y, &stride_grey);
...
EMSCRIPTEN_BINDINGS(libheif) {
  ...
  emscripten::function("heif_js_context_get_image_handle",
                       ...
                       emscripten::allow_raw_pointers());
  emscripten::function("heif_js_decode_image", &heif_js_decode_image,
                       emscripten::allow_raw_pointers());
  ...
  emscripten::enum_<heif_error_code>("heif_error_code")
      ...
      .value("heif_error_Memory_allocation_error",
  ...
  emscripten::enum_<heif_suberror_code>("heif_suberror_code")
      ...
      .value("heif_suberror_Ipma_box_references_nonexisting_property",
             heif_suberror_Ipma_box_references_nonexisting_property)
      .value("heif_suberror_No_properties_assigned_to_item",
      ...
      .value("heif_suberror_Invalid_overlay_data",
             heif_suberror_Invalid_overlay_data)
      .value("heif_suberror_Overlay_image_outside_of_canvas",
             heif_suberror_Overlay_image_outside_of_canvas)
      .value("heif_suberror_Auxiliary_image_type_unspecified",
             heif_suberror_Auxiliary_image_type_unspecified)
      .value("heif_suberror_No_or_invalid_primary_item",
             heif_suberror_No_or_invalid_primary_item)
      .value("heif_suberror_No_infe_box", heif_suberror_No_infe_box)
      .value("heif_suberror_Security_limit_exceeded",
             heif_suberror_Security_limit_exceeded)
      .value("heif_suberror_Unknown_color_profile_type",
             heif_suberror_Unknown_color_profile_type)
      .value("heif_suberror_Nonexisting_item_referenced",
             heif_suberror_Nonexisting_item_referenced)
      .value("heif_suberror_Null_pointer_argument",
             heif_suberror_Null_pointer_argument)
      .value("heif_suberror_Nonexisting_image_channel_referenced",
             heif_suberror_Nonexisting_image_channel_referenced)
      .value("heif_suberror_Unsupported_plugin_version",
      ...
      .value("heif_suberror_Unsupported_parameter",
             heif_suberror_Unsupported_parameter)
      .value("heif_suberror_Invalid_parameter_value",
             heif_suberror_Invalid_parameter_value)
      .value("heif_suberror_Unsupported_codec", heif_suberror_Unsupported_codec)
      .value("heif_suberror_Unsupported_image_type",
             heif_suberror_Unsupported_image_type)
      .value("heif_suberror_Unsupported_data_version",
             heif_suberror_Unsupported_data_version)
      .value("heif_suberror_Unsupported_color_conversion",
             heif_suberror_Unsupported_color_conversion)
      .value("heif_suberror_Unsupported_item_construction_method",
