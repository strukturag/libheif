...
struct heif_error convert_libde265_image_to_heif_image(
    struct libde265_decoder* decoder,
    const struct de265_image* de265img,
    struct heif_image** image) {
  struct heif_image* out_img;
  struct heif_error err = heif_image_create(
      de265_get_image_width(de265img, 0), de265_get_image_height(de265img, 0),
      heif_colorspace_YCbCr,  // TODO
      (heif_chroma)de265_get_chroma_format(de265img), &out_img);
  ...
  heif_channel channel2plane[3] = {heif_channel_Y, heif_channel_Cb,
                                   heif_channel_Cr};
  ...
  for (int c = 0; c < 3; c++) {
    ...
    int dst_stride;
    uint8_t* dst_mem =
        heif_image_get_plane(out_img, channel2plane[c], &dst_stride);
...
struct heif_error libde265_new_decoder(void** dec) {
  ...
  de265_set_parameter_bool(decoder->ctx, DE265_DECODER_PARAM_DISABLE_DEBLOCKING,
                           1);
...
struct heif_error libde265_v2_push_data(void* decoder_raw,
                                        const void* data,
                                        size_t size) {
  struct libde265_decoder* decoder = (struct libde265_decoder*)decoder_raw;
  ...
  while (ptr < size) {
    if (ptr + 4 > size) {
      struct heif_error err = {heif_error_Decoder_plugin_error,
                               heif_suberror_End_of_data, kEmptyString};
      return err;
    }

    // TODO: the size of the NAL unit length variable is defined in the hvcC
    // header.
    // We should not assume that it is always 4 bytes.
    uint32_t nal_size = (cdata[ptr] << 24) | (cdata[ptr + 1] << 16) |
                        (cdata[ptr + 2] << 8) | (cdata[ptr + 3]);
    ptr += 4;
...
struct heif_error libde265_v2_decode_image(void* decoder_raw,
                                           struct heif_image** out_img) {
  struct libde265_decoder* decoder = (struct libde265_decoder*)decoder_raw;
  ...
  if (action == de265_action_get_image) {
    ...
    if (img) {
      struct heif_error err =
          convert_libde265_image_to_heif_image(decoder, img, out_img);
  ...
  struct heif_error err = {heif_error_Decoder_plugin_error,
                           heif_suberror_Unspecified, kEmptyString};
  return err;
}

#else

struct heif_error libde265_v1_push_data(void* decoder_raw,
                                        const void* data,
                                        size_t size) {
  struct libde265_decoder* decoder = (struct libde265_decoder*)decoder_raw;
...
struct heif_error libde265_v1_decode_image(void* decoder_raw,
                                           struct heif_image** out_img) {
  struct libde265_decoder* decoder = (struct libde265_decoder*)decoder_raw;
  struct heif_error err = {heif_error_Ok, heif_suberror_Unspecified, kSuccess};
  ...
  do {
    ...
    if (decode_err != DE265_OK) {
      // printf("Error decoding: %s (%d)\n", de265_get_error_text(decode_err),
      // decode_err);
      break;
    }
...
static const struct heif_decoder_plugin decoder_libde265 {
  1, libde265_plugin_name, libde265_init_plugin, libde265_deinit_plugin,
      libde265_does_support_format, libde265_new_decoder, libde265_free_decoder,
      libde265_v2_push_data, libde265_v2_decode_image
};
...
static const struct heif_decoder_plugin decoder_libde265 {
  1, libde265_plugin_name, libde265_init_plugin, libde265_deinit_plugin,
      libde265_does_support_format, libde265_new_decoder, libde265_free_decoder,
      libde265_v1_push_data, libde265_v1_decode_image
};
