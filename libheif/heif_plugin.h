...
// ====================================================================================================
//  Decoder plugin API
//  In order to decode images in other formats than HEVC, additional compression
//  codecs can be
//  added as plugins. A plugin has to implement the functions specified in
//  heif_decoder_plugin
//  and the plugin has to be registered to the libheif library using
...
struct heif_decoder_plugin {
  ...
  // --- After pushing the data into the decoder, the decode functions may be
  // called only once.

  // Decode data into a full image. All data has to be pushed into the decoder
  // before calling this.
...
struct heif_encoder_plugin {
  ...
  // Short name of the encoder that can be used as command line parameter when
  // selecting an encoder.
  // Hence, it should stay stable and not contain any version numbers that will
  // change.
  const char* id_name;
  ...
  struct heif_error (*set_parameter_integer)(void* encoder,
                                             const char* name,
                                             int value);
  struct heif_error (*get_parameter_integer)(void* encoder,
                                             const char* name,
                                             int* value);
  struct heif_error (*set_parameter_boolean)(void* encoder,
                                             const char* name,
                                             int value);
  struct heif_error (*get_parameter_boolean)(void* encoder,
                                             const char* name,
                                             int* value);
  struct heif_error (*set_parameter_string)(void* encoder,
                                            const char* name,
                                            const char* value);
  struct heif_error (*get_parameter_string)(void* encoder,
                                            const char* name,
                                            char* value,
                                            int value_size);

  // Replace the input colorspace/chroma with the one that is supported by the
  // encoder and that
  ...
  // Encode an image.
  // After pushing an image into the encoder, you should call
  // get_compressed_data() to
  // get compressed data until it returns a NULL data pointer.
  struct heif_error (*encode_image)(void* encoder,
                                    const struct heif_image* image,
                                    enum heif_image_input_class image_class);

  // Get a packet of decoded data. The data format depends on the codec.
  // For HEVC, each packet shall contain exactly one NAL, starting with the NAL
  // header without startcode.
  struct heif_error (*get_compressed_data)(void* encoder,
                                           uint8_t** data,
                                           int* size,
...
// Names for standard parameters. These should only be used by the encoder
// plugins.
