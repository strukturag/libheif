...
void HeifPixelImage::create(int width,
                            int height,
...
void HeifPixelImage::add_plane(heif_channel channel,
                               int width,
                               int height,
                               int bit_depth) {
  assert(bit_depth >= 1);
...
const uint8_t* HeifPixelImage::get_plane(enum heif_channel channel,
...
void HeifPixelImage::copy_new_plane_from(
  ...
  add_plane(dst_channel, width, height,
...
void HeifPixelImage::fill_new_plane(heif_channel dst_channel,
                                    uint8_t value,
                                    int width,
                                    int height) {
...
void HeifPixelImage::transfer_plane_from_image_as(
    std::shared_ptr<HeifPixelImage> source,
    heif_channel src_channel,
    heif_channel dst_channel) {
...
std::shared_ptr<HeifPixelImage> HeifPixelImage::convert_colorspace(
...
std::shared_ptr<HeifPixelImage> HeifPixelImage::convert_YCbCr420_to_RGB()
    const {
...
std::shared_ptr<HeifPixelImage> HeifPixelImage::convert_YCbCr420_to_RGB24()
    const {
  if (get_bits_per_pixel(heif_channel_Y) != 8 ||
      get_bits_per_pixel(heif_channel_Cb) != 8 ||
      get_bits_per_pixel(heif_channel_Cr) != 8) {
  ...
  for (y = 0; y < m_height; y++) {
    for (x = 0; x < m_width; x++) {
      ...
      out_p[y * out_p_stride + 3 * x + 1] =
          clip(yv - 0.344136f * cb - 0.714136f * cr);
...
std::shared_ptr<HeifPixelImage> HeifPixelImage::convert_YCbCr420_to_RGB32()
    const {
  if (get_bits_per_pixel(heif_channel_Y) != 8 ||
      get_bits_per_pixel(heif_channel_Cb) != 8 ||
      get_bits_per_pixel(heif_channel_Cr) != 8) {
...
std::shared_ptr<HeifPixelImage> HeifPixelImage::convert_RGB_to_RGB24_32()
    const {
  bool has_alpha = has_channel(heif_channel_Alpha);
  ...
  outimg->add_plane(heif_channel_interleaved, m_width, m_height,
                    has_alpha ? 32 : 24);
...
std::shared_ptr<HeifPixelImage> HeifPixelImage::convert_mono_to_YCbCr420()
    const {
  auto outimg = std::make_shared<HeifPixelImage>();
  ...
  // outimg->transfer_plane_from_image_as(shared_from_this(), heif_channel_Y,
  // heif_channel_Y);
  ...
  for (int y = 0; y < m_height; y++) {
    memcpy(out_y + y * out_y_stride, in_y + y * in_y_stride, m_width);
  }

  return outimg;
}

std::shared_ptr<HeifPixelImage> HeifPixelImage::convert_mono_to_RGB(
    int bpp) const {
  if (get_bits_per_pixel(heif_channel_Y) != 8) {
    return nullptr;
  }
...
std::shared_ptr<HeifPixelImage> HeifPixelImage::convert_RGB24_32_to_YCbCr420()
    const {
  auto outimg = std::make_shared<HeifPixelImage>();
  ...
  if (!has_alpha) {
    for (int y = 0; y < m_height; y++) {
      for (int x = 0; x < m_width; x++) {
        ...
        out_y[y * out_y_stride + x] =
            clip(r * 0.299f + g * 0.587f + b * 0.114f);
      }
    }
    ...
    for (int y = 0; y < m_height; y += 2) {
      for (int x = 0; x < m_width; x += 2) {
        ...
        uint8_t g = in_p[y * in_stride + x * 3 + 1];
        uint8_t b = in_p[y * in_stride + x * 3 + 2];
        out_cb[(y / 2) * out_cb_stride + (x / 2)] =
            clip(128 - r * 0.168736f - g * 0.331264f + b * 0.5f);
        out_cr[(y / 2) * out_cb_stride + (x / 2)] =
            clip(128 + r * 0.5f - g * 0.418688f - b * 0.081312f);
      }
    }
  } else {
    for (int y = 0; y < m_height; y++) {
...
Error HeifPixelImage::rotate_ccw(int angle_degrees,
  ...
  for (const auto& plane_pair : m_planes) {
    ...
    out_img->add_plane(channel, out_plane_width, out_plane_height,
                       plane.bit_depth);
...
Error HeifPixelImage::crop(int left,
                           int right,
                           int top,
                           int bottom,
  ...
  for (const auto& plane_pair : m_planes) {
    ...
    out_img->add_plane(channel, plane_right - plane_left + 1,
                       plane_bottom - plane_top + 1, plane.bit_depth);
...
Error HeifPixelImage::fill_RGB_16bit(uint16_t r,
                                     uint16_t g,
                                     uint16_t b,
                                     uint16_t a) {
...
Error HeifPixelImage::overlay(std::shared_ptr<HeifPixelImage>& overlay,
                              int dx,
                              int dy) {
  ...
  for (heif_channel channel : channels) {
    ...
    // if overlay image is completely outside at left border, do not copy
    // anything.
    if (in_w <= in_x0 || in_h <= in_y0) {
      return Error(heif_error_Invalid_input,
                   heif_suberror_Overlay_image_outside_of_canvas,
    ...
    for (int y = in_y0; y < in_h; y++) {
      if (!has_alpha) {
        memcpy(out_p + out_x0 + (out_y0 + y - in_y0) * out_stride,
               in_p + in_x0 + y * in_stride, in_w - in_x0);
      } else {
        for (int x = in_x0; x < in_w; x++) {
          uint8_t* outptr =
              &out_p[out_x0 + (out_y0 + y - in_y0) * out_stride + x];
          uint8_t in_val = in_p[in_x0 + y * in_stride + x];
          uint8_t alpha_val = alpha_p[in_x0 + y * in_stride + x];

          *outptr = (uint8_t)(
              (in_val * alpha_val + *outptr * (255 - alpha_val)) / 255);
        }
      }
    }
  }
...
Error HeifPixelImage::scale_nearest_neighbor(
  ...
  for (const auto& plane_pair : m_planes) {
    ...
    out_img->add_plane(channel, out_w, out_h, plane.bit_depth);
    ...
    for (int y = 0; y < out_h; y++) {
      ...
      if (bpp == 1) {
        ...
        for (int x = 0; x < out_w; x++) {
          ...
          for (int b = 0; b < bpp; b++) {
            out_data[y * out_stride + bpp * x + b] =
                in_data[iy * in_stride + bpp * ix + b];
          }
        }
      }
    }
