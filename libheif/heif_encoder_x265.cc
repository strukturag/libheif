...
const char* x265_plugin_name() {
  ...
  if (strlen(x265_version_str) + strlen(plugin_name) + 4 <
...
static const struct heif_encoder_parameter*
    x265_encoder_parameter_ptrs[MAX_NPARAMETERS + 1];
...
struct heif_error x265_new_encoder(void** enc) {
  ...
  // encoder has to be allocated in x265_encode_image, because it needs to know
  // the image size
...
void x265_free_encoder(void* encoder_raw) {
  struct x265_encoder_struct* encoder =
      (struct x265_encoder_struct*)encoder_raw;
...
struct heif_error x265_set_parameter_logging_level(void* encoder_raw,
                                                   int logging) {
  struct x265_encoder_struct* encoder =
      (struct x265_encoder_struct*)encoder_raw;
...
struct heif_error x265_get_parameter_logging_level(void* encoder_raw,
                                                   int* loglevel) {
  struct x265_encoder_struct* encoder =
      (struct x265_encoder_struct*)encoder_raw;
...
struct heif_error x265_set_parameter_integer(void* encoder_raw,
                                             const char* name,
                                             int value) {
  struct x265_encoder_struct* encoder =
      (struct x265_encoder_struct*)encoder_raw;
...
struct heif_error x265_get_parameter_integer(void* encoder_raw,
                                             const char* name,
                                             int* value) {
  struct x265_encoder_struct* encoder =
      (struct x265_encoder_struct*)encoder_raw;
...
struct heif_error x265_set_parameter_boolean(void* encoder,
                                             const char* name,
                                             int value) {
...
struct heif_error x265_get_parameter_boolean(void* encoder,
                                             const char* name,
                                             int* value) {
  if (strcmp(name, heif_encoder_parameter_name_lossless) == 0) {
    return x265_get_parameter_lossless(encoder, value);
...
struct heif_error x265_set_parameter_string(void* encoder_raw,
                                            const char* name,
                                            const char* value) {
  struct x265_encoder_struct* encoder =
      (struct x265_encoder_struct*)encoder_raw;
...
struct heif_error x265_get_parameter_string(void* encoder_raw,
                                            const char* name,
                                            char* value,
                                            int value_size) {
  struct x265_encoder_struct* encoder =
...
static void x265_set_default_parameters(void* encoder) {
  for (const struct heif_encoder_parameter** p = x265_encoder_parameter_ptrs;
       *p; p++) {
    const struct heif_encoder_parameter* param = *p;

    switch (param->type) {
    case heif_encoder_parameter_type_integer:
      x265_set_parameter_integer(encoder, param->name,
                                 param->integer.default_value);
      break;
    case heif_encoder_parameter_type_boolean:
      x265_set_parameter_boolean(encoder, param->name,
                                 param->boolean.default_value);
      break;
    case heif_encoder_parameter_type_string:
      x265_set_parameter_string(encoder, param->name,
...
void x265_query_input_colorspace(heif_colorspace* colorspace,
                                 heif_chroma* chroma) {
  *colorspace = heif_colorspace_YCbCr;
  *chroma = heif_chroma_420;
}

struct heif_error x265_encode_image(void* encoder_raw,
  ...
  x265_param_default_preset(param, encoder->preset.c_str(),
                            encoder->tune.c_str());
  ...
  pic->planes[0] = (void*)heif_image_get_plane_readonly(image, heif_channel_Y,
                                                        &pic->stride[0]);
  pic->planes[1] = (void*)heif_image_get_plane_readonly(image, heif_channel_Cb,
                                                        &pic->stride[1]);
  pic->planes[2] = (void*)heif_image_get_plane_readonly(image, heif_channel_Cr,
                                                        &pic->stride[2]);
  pic->bitDepth = 8;
  ...
  int result = x265_encoder_encode(encoder->encoder, &encoder->nals,
                                   &encoder->num_nals, pic, NULL);
  (void)result;
...
struct heif_error x265_get_compressed_data(void* encoder_raw,
                                           uint8_t** data,
                                           int* size,
...
static const struct heif_encoder_plugin encoder_plugin_x265 {
  /* plugin_api_version */ 1,
      ...
      /* set_parameter_boolean */ x265_set_parameter_integer,  // boolean also
                                                               // maps to
                                                               // integer
                                                               // function
      /* get_parameter_boolean */ x265_get_parameter_integer,  // boolean also
                                                               // maps to
                                                               // integer
                                                               // function
