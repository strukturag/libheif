...
static int32_t readvec_signed(const std::vector<uint8_t>& data,
                              int& ptr,
                              int len) {
...
Error ImageGrid::parse(const std::vector<uint8_t>& data) {
  ...
  if (field_size == 32) {
    ...
    m_output_width = ((data[4] << 8) | (data[5]));

    m_output_height = ((data[6] << 8) | (data[7]));
  }
...
Error ImageOverlay::parse(size_t num_images, const std::vector<uint8_t>& data) {
  Error eofError(heif_error_Invalid_input, heif_suberror_Invalid_grid_data,
  ...
  if (m_version != 0) {
    ...
    sstr << "Overlay image data version " << ((int)m_version)
         << " is not implemented yet";

    return Error(heif_error_Unsupported_feature,
                 heif_suberror_Unsupported_data_version, sstr.str());
  }
...
std::string ImageOverlay::dump() const {
  ...
  sstr << "version: " << ((int)m_version) << "\n"
       ...
       << m_background_color[1] << ";" << m_background_color[2] << ";"
...
void ImageOverlay::get_offset(size_t image_index,
                              int32_t* x,
                              int32_t* y) const {
...
const struct heif_decoder_plugin* HeifContext::get_decoder(
    enum heif_compression_format type) const {
...
static bool item_type_is_image(const std::string& item_type) {
  return (item_type == "hvc1" || item_type == "grid" || item_type == "iden" ||
          item_type == "iovl");
}
...
Error HeifContext::interpret_heif_file() {
  ...
  for (heif_item_id id : image_IDs) {
    ...
    if (!infe_box) {
      // TODO(farindk): Should we return an error instead of skipping the
      // invalid id?
      continue;
    }
  ...
  // --- remove thumbnails from top-level images and assign to their respective
  // image
  ...
  if (iref_box) {
    ...
    for (auto& pair : m_all_images) {
      ...
      std::vector<Box_iref::Reference> references =
          iref_box->get_references_from(image->get_id());
      ...
      for (const Box_iref::Reference& ref : references) {
        ...
        if (type == fourcc("thmb")) {
          ...
          //     check whether it is an alpha channel and attach to the main
          //     image if yes
          ...
          if (auxC_property->get_aux_type() == "urn:mpeg:hevc:2015:auxid:2") {
            ...
            for (auto& msg : sei_messages) {
              auto depth_msg = std::dynamic_pointer_cast<
                  SEIMessage_depth_representation_info>(msg);
              if (depth_msg) {
  ...
  for (auto& pair : m_all_images) {
    ...
    if (infe->get_item_type() == "hvc1") {
      ...
      if (!ipco->get_property_for_item_ID(image->get_id(), ipma,
                                          fourcc("hvcC"))) {
    ...
    for (const auto& prop : properties) {
      ...
      if (ispe) {
        ...
        if (width >= static_cast<uint32_t>(MAX_IMAGE_WIDTH) ||
          ...
          std::stringstream sstr;
          sstr << "Image size " << width << "x" << height
               << " exceeds the maximum image size " << MAX_IMAGE_WIDTH << "x"
               << MAX_IMAGE_HEIGHT << "\n";

          return Error(heif_error_Memory_allocation_error,
                       heif_suberror_Security_limit_exceeded, sstr.str());
        }
      ...
      if (ispe_read) {
        ...
        if (irot) {
          if (irot->get_rotation() == 90 || irot->get_rotation() == 270) {
            // swap width and height
            image->set_resolution(image->get_height(), image->get_width());
          }
        }
      }
      ...
      if (colr) {
        ...
        if (primary_is_grid && !primary_colr_set && image->is_grid_item()) {
  ...
  for (heif_item_id id : image_IDs) {
    ...
    if (item_type == "Exif" ||
      ...
      Error err =
          m_heif_file->get_compressed_image_data(id, &(metadata->m_data));
      if (err) {
        return err;
      }
      ...
      if (iref_box) {
        std::vector<Box_iref::Reference> references =
            iref_box->get_references_from(id);
...
HeifContext::Image::Image(HeifContext* context, heif_item_id id)
    : m_heif_context(context), m_id(id) {}
...
Error HeifContext::Image::decode_image(
  ...
  heif_chroma target_chroma =
      (chroma == heif_chroma_undefined ? img->get_chroma_format() : chroma);
  heif_colorspace target_colorspace =
      (colorspace == heif_colorspace_undefined ? img->get_colorspace()
                                               : colorspace);
...
Error HeifContext::decode_image(
    heif_item_id ID,
    std::shared_ptr<HeifPixelImage>& img,
    const struct heif_decoding_options* options) const {
  ...
  if (image_type == "hvc1") {
    const struct heif_decoder_plugin* decoder_plugin =
        get_decoder(heif_compression_HEVC);
    if (!decoder_plugin) {
      return Error(heif_error_Unsupported_feature,
  ...
  // TODO: this if statement is probably wrong. When we have a tiled image with
  // alpha
  // channel, then the alpha images should be associated with their respective
  // tiles.
  ...
  if (m_all_images.find(ID) != m_all_images.end()) {
    ...
    if (alpha_image) {
      ...
      // BUT: is there any indication in the standard that the alpha channel
      // should have the same size?

      img->transfer_plane_from_image_as(alpha, heif_channel_Y,
                                        heif_channel_Alpha);
    }
  }
  ...
  if (!options || options->ignore_transformations == false) {
    ...
    for (const auto& property : properties) {
      ...
      if (mirror) {
        error = img->mirror_inplace(mirror->get_mirror_axis() ==
                                    Box_imir::MirrorAxis::Horizontal);
        if (error) {
          return error;
        }
      }
...
// TODO: this function only works with YCbCr images, chroma 4:2:0, and 8 bpp at
// the moment
...
Error HeifContext::decode_full_grid_image(
  ...
  if (!iref_box) {
    return Error(heif_error_Invalid_input, heif_suberror_No_iref_box,
                 "No iref box available, but needed for grid image");
  }

  std::vector<heif_item_id> image_references =
      iref_box->get_references(ID, fourcc("dimg"));
  ...
  if ((int)image_references.size() != grid.get_rows() * grid.get_columns()) {
    ...
    return Error(heif_error_Invalid_input, heif_suberror_Missing_grid_images,
                 sstr.str());
  }
  ...
  // --- determine output image chroma size and make sure all tiles have same
  // chroma

  assert(!image_references.empty());
  heif_item_id some_tile_id = image_references[0];
  heif_chroma tile_chroma =
      m_heif_file->get_image_chroma_from_configuration(some_tile_id);

  const int cw = w / chroma_h_subsampling(tile_chroma);
  const int ch = h / chroma_v_subsampling(tile_chroma);

  for (heif_item_id tile_id : image_references) {
    if (m_heif_file->get_image_chroma_from_configuration(tile_id) !=
        tile_chroma) {
      return Error(heif_error_Invalid_input, heif_suberror_Invalid_grid_data,
  ...
  for (int y = 0; y < grid.get_rows(); y++) {
    ...
    for (int x = 0; x < grid.get_columns(); x++) {
      ...
      x0 += src_width;
      tile_height =
          src_height;  // TODO: check that all tiles have the same height
  ...
  for (heif_channel channel : channels) {
    ...
    for (int py = 0; py < copy_height; py++) {
      memcpy(out_data + xs + (ys + py) * out_stride,
             tile_data + py * tile_stride, copy_width);
    }
  }
...
Error HeifContext::decode_derived_image(
    heif_item_id ID,
...
Error HeifContext::decode_overlay_image(
    heif_item_id ID,
    std::shared_ptr<HeifPixelImage>& img,
  ...
  */
  ...
  if (image_references.size() != overlay.get_num_offsets()) {
    return Error(heif_error_Invalid_input, heif_suberror_Invalid_overlay_data,
                 "Number of image offsets does not match the number of image "
                 "references");
  }
  ...
  // TODO: seems we always have to compose this in RGB since the background
  ...
  err = img->fill_RGB_16bit(bkg_color[0], bkg_color[1], bkg_color[2],
                            bkg_color[3]);
  if (err) {
    return err;
  }
  ...
  for (size_t i = 0; i < image_references.size(); i++) {
    ...
    overlay_img =
        overlay_img->convert_colorspace(heif_colorspace_RGB, heif_chroma_444);
    if (!overlay_img) {
      return Error(heif_error_Unsupported_feature,
                   heif_suberror_Unsupported_color_conversion);
    }
...
static std::shared_ptr<HeifPixelImage> create_alpha_image_from_image_alpha_channel(
    const std::shared_ptr<HeifPixelImage> image) {
  // --- generate alpha image
  // TODO: can we directly code a monochrome image instead of the dummy color
  // channels?
  ...
  alpha_image->fill_new_plane(heif_channel_Cb, 128, chroma_width,
                              chroma_height);
  alpha_image->fill_new_plane(heif_channel_Cr, 128, chroma_width,
                              chroma_height);

  return alpha_image;
}

void HeifContext::Image::set_preencoded_hevc_image(
  ...
  int prev_start_code_start =
      -1;  // init to an invalid value, will always be overwritten before use
  ...
  for (;;) {
    ...
    if (dump_nal) {
      if (first) {
        ...
        memcpy(nal_data.data(), data.data() + prev_start_code_start + 3,
               length);
        ...
        default: {
          ...
          memcpy(nal_data_with_size.data() + 4, nal_data.data(),
                 nal_data.size());
          ...
          m_heif_context->m_heif_file->append_iloc_data(m_id,
                                                        nal_data_with_size);
        } break;
        }
      }
...
Error HeifContext::encode_image(std::shared_ptr<HeifPixelImage> pixel_image,
  ...
  case heif_compression_HEVC: {
    ...
    error = out_image->encode_image_as_hevc(pixel_image, encoder, options,
...
Error HeifContext::Image::encode_image_as_hevc(
  ...
  */
  ...
  if (options->save_alpha_channel && image->has_channel(heif_channel_Alpha)) {
    ...
    heif_item_id alpha_image_id =
        m_heif_context->m_heif_file->add_new_image("hvc1");

    std::shared_ptr<HeifContext::Image> heif_alpha_image;
    heif_alpha_image = std::make_shared<Image>(m_heif_context, alpha_image_id);

    Error error = heif_alpha_image->encode_image_as_hevc(
        alpha_image, encoder, options, heif_image_input_class_alpha);
    if (error) {
      return error;
    }

    m_heif_context->m_heif_file->add_iref_reference(alpha_image_id,
                                                    fourcc("auxl"), {m_id});
    m_heif_context->m_heif_file->set_auxC_property(
        alpha_image_id, "urn:mpeg:hevc:2015:auxid:1");
  }
  ...
  for (;;) {
    ...
    default:
      m_heif_context->m_heif_file->append_iloc_data_with_4byte_size(m_id, data,
                                                                    size);
    }
  }
...
Error HeifContext::assign_thumbnail(std::shared_ptr<Image> master_image,
  ...
  m_heif_file->add_iref_reference(thumbnail_image->get_id(), fourcc("thmb"),
                                  {master_image->get_id()});

  return Error::Ok;
}

Error HeifContext::encode_thumbnail(
    std::shared_ptr<HeifPixelImage> image,
    struct heif_encoder* encoder,
    const struct heif_encoding_options* options,
    int bbox_size,
  ...
  if (orig_width <= bbox_size && orig_height <= bbox_size) {
    // original image is smaller than thumbnail size -> do not encode any
    // thumbnail
  ...
  error =
      image->scale_nearest_neighbor(thumbnail_image, thumb_width, thumb_height);
  if (error) {
    return error;
  }

  error = encode_image(thumbnail_image, encoder, options,
                       heif_image_input_class_thumbnail, out_thumbnail_handle);
  if (error) {
    return error;
  }
...
Error HeifContext::add_exif_metadata(std::shared_ptr<Image> master_image,
                                     const void* data,
                                     int size) {
  ...
  while (offset + 4 < (unsigned int)size) {
    if (!memcmp((uint8_t*)data + offset, tiffmagic1, 4))
      break;
    if (!memcmp((uint8_t*)data + offset, tiffmagic2, 4))
      break;
    offset++;
  }
  ...
  // create an infe box describing what kind of data we are storing (this also
  // creates a new ID)
  ...
  m_heif_file->add_iref_reference(metadata_id, fourcc("cdsc"),
                                  {master_image->get_id()});

  // copy the Exif data into the file, store the pointer to it in an iloc box
  // entry
...
Error HeifContext::add_XMP_metadata(std::shared_ptr<Image> master_image,
                                    const void* data,
                                    int size) {
  // create an infe box describing what kind of data we are storing (this also
  // creates a new ID)
  ...
  // copy the XMP data into the file, store the pointer to it in an iloc box
  // entry
