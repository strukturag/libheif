...
uint32_t from_fourcc(const char* string) {
  return ((string[0] << 24) | (string[1] << 16) | (string[2] << 8) |
          (string[3]));
}
...
heif::Error heif::BoxHeader::prepend_header(StreamWriter& writer,
                                            size_t box_start) const {
  ...
  writer.set_position_to_end();  // Note: should we move to the end of the box
...
std::string BoxHeader::dump(Indent& indent) const {
  ...
  sstr << indent << "size: " << get_box_size()
       << "   (header size: " << get_header_size() << ")\n";
...
Error Box::read(BitstreamRange& range, std::shared_ptr<heif::Box>* result) {
  ...
  if (hdr.get_box_size() < hdr.get_header_size()) {
    std::stringstream sstr;
    sstr << "Box size (" << hdr.get_box_size()
         << " bytes) smaller than header size (" << hdr.get_header_size()
         << " bytes)";
  ...
  auto status = range.wait_for_available_bytes(hdr.get_box_size() -
                                               hdr.get_header_size());
  if (status != StreamReader::size_reached) {
    // TODO: return recoverable error at timeout
    return Error(heif_error_Invalid_input, heif_suberror_End_of_data);
...
std::vector<std::shared_ptr<Box>> Box::get_child_boxes(
    uint32_t short_type) const {
...
Error Box::read_children(BitstreamRange& range, int max_number) {
  ...
  while (!range.eof() && !range.error()) {
    ...
    if (m_children.size() > MAX_CHILDREN_PER_BOX) {
      std::stringstream sstr;
      sstr << "Maximum number of child boxes " << MAX_CHILDREN_PER_BOX
           << " exceeded.";
    ...
    // count the new child and end reading new children when we reached the
    // expected number
...
Error Box_iloc::parse(BitstreamRange& range) {
  ...
  */
  ...
  if (item_count > MAX_ILOC_ITEMS) {
    std::stringstream sstr;
    sstr << "iloc box contains " << item_count
         << " items, which exceeds the security limit of " << MAX_ILOC_ITEMS
         << " items.";
  ...
  for (int i = 0; i < item_count; i++) {
    ...
    if (extent_count > MAX_ILOC_EXTENTS_PER_ITEM) {
      std::stringstream sstr;
      sstr << "Number of extents in iloc box (" << extent_count
           << ") exceeds security limit (" << MAX_ILOC_EXTENTS_PER_ITEM
           << ")\n";
...
std::string Box_iloc::dump(Indent& indent) const {
  ...
  for (const Item& item : m_items) {
    sstr << indent << "item ID: " << item.item_ID << "\n"
         << indent
         << "  construction method: " << ((int)item.construction_method) << "\n"
  ...
  for (const auto& extent : item.extents) {
    if (item.construction_method == 0) {
      ...
      if (MAX_MEMORY_BLOCK_SIZE - old_size < extent.length) {
        std::stringstream sstr;
        sstr << "iloc box contained " << extent.length
             << " bytes, total memory size would be "
             << (old_size + extent.length)
             << " bytes, exceeding the security limit of "
      ...
      StreamReader::grow_status status = istr->wait_for_file_size(
          extent.offset + item.base_offset + extent.length);
      if (status == StreamReader::size_beyond_eof) {
        // Out-of-bounds
        // TODO: I think we should not clear this. Maybe we want to try reading
        // again later and
        ...
        // TODO: maybe we should introduce some 'Recoverable error' instead of
        // 'Invalid input'
        return Error(heif_error_Invalid_input, heif_suberror_End_of_data);
      }
      ...
      success = istr->read((char*)dest->data() + old_size,
                           static_cast<size_t>(extent.length));
      assert(success);
    } else if (item.construction_method == 1) {
      if (!idat) {
        return Error(heif_error_Invalid_input, heif_suberror_No_idat_box,
                     "idat box referenced in iref box is not present in file");
      }

      idat->read_data(istr, extent.offset + item.base_offset, extent.length,
                      *dest);
    } else {
      std::stringstream sstr;
      sstr << "Item construction method " << item.construction_method
           << " not implemented";
      return Error(heif_error_Unsupported_feature, heif_suberror_No_idat_box,
                   sstr.str());
    }
  }
...
Error Box_ipco::get_properties_for_item_ID(
    uint32_t itemID,
    const std::shared_ptr<class Box_ipma>& ipma,
    std::vector<Property>& out_properties) const {
  const std::vector<Box_ipma::PropertyAssociation>* property_assoc =
      ipma->get_properties_for_item_ID(itemID);
  if (property_assoc == nullptr) {
    std::stringstream sstr;
    sstr << "Item (ID=" << itemID
         << ") has no properties assigned to it in ipma box";

    return Error(heif_error_Invalid_input,
                 heif_suberror_No_properties_assigned_to_item, sstr.str());
  }
  ...
  for (const Box_ipma::PropertyAssociation& assoc : *property_assoc) {
    if (assoc.property_index > allProperties.size()) {
      std::stringstream sstr;
      sstr << "Nonexisting property (index=" << assoc.property_index
           << ") for item "
...
std::shared_ptr<Box> Box_ipco::get_property_for_item_ID(
    heif_item_id itemID,
    const std::shared_ptr<class Box_ipma>& ipma,
...
const std::vector<Box_ipma::PropertyAssociation>* Box_ipma::
    get_properties_for_item_ID(uint32_t itemID) const {
...
std::string Box_clap::dump(Indent& indent) const {
  ...
  sstr << indent << "offset: " << m_horizontal_offset.numerator << "/"
       ...
       << m_vertical_offset.numerator << "/" << m_vertical_offset.denominator
       << "\n";
...
Error Box_iref::write(StreamWriter& writer) const {
  ...
  for (const auto& ref : m_references) {
    uint32_t box_size =
        uint32_t(4 + 4 + 2 + id_size * (1 + ref.to_item_ID.size()));
...
std::string Box_iref::dump(Indent& indent) const {
  ...
  for (const auto& ref : m_references) {
    sstr << indent << "reference with type '" << ref.header.get_type_string()
         << "'"
         << " from ID: " << ref.from_item_ID << " to IDs: ";
...
std::vector<Box_iref::Reference> Box_iref::get_references_from(
...
std::vector<uint32_t> Box_iref::get_references(uint32_t itemID,
                                               uint32_t ref_type) const {
  for (const Reference& ref : m_references) {
...
void Box_iref::add_reference(heif_item_id from_id,
                             uint32_t type,
                             std::vector<heif_item_id> to_ids) {
  Reference ref;
...
std::string Box_hvcC::dump(Indent& indent) const {
  ...
  sstr << indent << "general_level_idc: " << ((int)c.general_level_idc) << "\n"
       << indent
       << "min_spatial_segmentation_idc: " << c.min_spatial_segmentation_idc
       << "\n"
...
std::string Box_idat::dump(Indent& indent) const {
  ...
  sstr << indent
       << "number of data bytes: " << get_box_size() - get_header_size()
       << "\n";
...
Error Box_idat::read_data(std::shared_ptr<StreamReader> istr,
  ...
  if (MAX_MEMORY_BLOCK_SIZE - curr_size < length) {
    std::stringstream sstr;
    sstr << "idat box contained " << length
         << " bytes, total memory size would be " << (curr_size + length)
         << " bytes, exceeding the security limit of " << MAX_MEMORY_BLOCK_SIZE
         << " bytes";
  ...
  StreamReader::grow_status status =
      istr->wait_for_file_size((int64_t)m_data_start_pos + start + length);
  if (status == StreamReader::size_beyond_eof ||
