...
class StreamReader {
 public:
  ...
  enum grow_status {
    ...
    size_beyond_eof  // size has not been reached and never will. The file has
...
class StreamReader_memory : public StreamReader {
 ...
 private:
  ...
  // if we made a copy of the data, we store a pointer to the owned memory area
  // here
...
class BitstreamRange {
 public:
  ...
  // If you don't, you have to make sure that you do not read past the available
  // data.
  ...
  void skip_to_end_of_file() {
    // we do not actually move the file position here (because the stream may
    // still be incomplete),
  ...
  bool eof() const { return m_remaining == 0; }
