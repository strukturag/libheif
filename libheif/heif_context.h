...
class HeifContext : public ErrorBuffer {
 public:
  ...
  class Image : public ErrorBuffer {
   public:
    ...
    void set_depth_representation_info(
        struct heif_depth_representation_info& info) {
    ...
    const struct heif_depth_representation_info& get_depth_representation_info()
        const {
    ...
    void set_color_profile(std::shared_ptr<color_profile> profile) {
      m_color_profile = profile;
    };

    bool is_grid_item() {
      return !m_is_primary && !m_is_primary && !m_is_alpha_channel &&
             !m_is_depth_channel;
    };
  ...
  // Note that this is no valid HEIF file, since some boxes (e.g. pitm) are
  // generated, but
  ...
  Error add_exif_metadata(std::shared_ptr<Image> master_image,
                          const void* data,
                          int size);

  Error add_XMP_metadata(std::shared_ptr<Image> master_image,
                         const void* data,
                         int size);
 ...
 private:
  const struct heif_decoder_plugin* get_decoder(
