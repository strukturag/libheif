...
#define fourcc(id)                                                             \
  (((uint32_t)(id[0]) << 24) | (id[1] << 16) | (id[2] << 8) | (id[3]))
...
class Box_ipco : public Box {
 public:
  ...
  std::shared_ptr<Box> get_property_for_item_ID(
      heif_item_id itemID,
      const std::shared_ptr<class Box_ipma>&,
...
class Box_ipma : public Box {
 public:
  ...
  const std::vector<PropertyAssociation>* get_properties_for_item_ID(
...
class Box_imir : public Box {
 public:
  ...
  enum class MirrorAxis : uint8_t { Vertical = 0, Horizontal = 1 };
...
class Box_clap : public Box {
 public:
  ...
  int right_rounded(
      int image_width) const;  // last column that is part of the cropped image
  int top_rounded(int image_height) const;  // first row
  int bottom_rounded(
      int image_height) const;  // last row included in the cropped image
...
class Box_iref : public Box {
 public:
  ...
  std::vector<heif_item_id> get_references(heif_item_id itemID,
                                           uint32_t ref_type) const;

  std::vector<Reference> get_references_from(heif_item_id itemID) const;

  void add_reference(heif_item_id from_id,
                     uint32_t type,
...
class Box_hvcC : public Box {
 public:
  ...
  struct configuration {
    ...
    std::bitset<NUM_CONSTRAINT_INDICATOR_FLAGS>
...
class Box_colr : public Box {
 public:
  ...
  std::shared_ptr<color_profile> get_color_profile() const {
    return m_color_profile;
  }
  void set_color_profile(std::shared_ptr<color_profile> prof) {
