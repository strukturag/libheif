...
int main(int argc, char** argv) {
  ...
  err = heif_context_read_from_file(context.get(), input_filename.c_str(),
                                    nullptr);
  if (err.code != 0) {
  ...
  int nThumbnails = heif_image_handle_get_list_of_thumbnail_IDs(
      image_handle, &thumbnail_ID, 1);
  if (nThumbnails > 0) {
    struct heif_image_handle* thumbnail_handle;
    err = heif_image_handle_get_thumbnail(image_handle, thumbnail_ID,
                                          &thumbnail_handle);
    if (err.code) {
      std::cerr << "Could not read HEIF image : " << err.message << "\n";
      return 1;
  ...
  err = heif_decode_image(image_handle, &image, encoder->colorspace(false),
                          encoder->chroma(false), NULL);
  if (err.code) {
  ...
  if (input_width > size || input_height > size) {
    ...
    err = heif_image_scale_image(image, &scaled_image, thumbnail_width,
