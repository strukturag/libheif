...
void show_help(const char* argv0) {
  ...
  fprintf(stderr, "  -d, --decode-img ID  decode image and output raw pixel "
                  "data of all planes\n");
...
int main(int argc, char** argv) {
  ...
  std::vector<std::pair<heif_item_id, heif_item_id>>
      metadata_IDs;  // first: image, second: metadata
  ...
  try {
    ...
    for (auto id : image_IDs) {
      ...
      heif::Image img =
          handle.decode_image(heif_colorspace_undefined, heif_chroma_undefined);
