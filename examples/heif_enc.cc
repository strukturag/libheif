...
void show_help(const char* argv0) {
  std::cerr
      << " heif-enc  libheif version: " << heif_get_version() << "\n"
      << "----------------------------------------\n"
      ...
      << "  -t, --thumb #   generate thumbnail with maximum size # (default: "
         "off)\n"
      << "      --no-alpha  do not save alpha channel\n"
      << "      --no-thumb-alpha  do not save alpha channel in thumbnail "
         "image\n"
      << "  -o, --output    output filename (optional)\n"
      << "  -v, --verbose   enable logging output (more -v will increase "
         "logging level)\n"
...
std::shared_ptr<heif_image> loadJPEG(const char* filename) {
  ...
  // ### Code copied from LibVideoGfx and slightly modified to use
  // HeifPixelImage
  ...
  if (cinfo.jpeg_color_space == JCS_GRAYSCALE) {
    ...
    struct heif_error err = heif_image_create(
        cinfo.output_width, cinfo.output_height, heif_colorspace_YCbCr,
        heif_chroma_monochrome, &image);
    (void)err;
    // TODO: handle error

    heif_image_add_plane(image, heif_channel_Y, cinfo.output_width,
    ...
    while (cinfo.output_scanline < cinfo.output_height) {
      ...
      memcpy(py + (cinfo.output_scanline - 1) * y_stride, buffer,
             cinfo.output_width);
    }
    ...
    struct heif_error err =
        heif_image_create(cinfo.output_width, cinfo.output_height,
                          heif_colorspace_YCbCr, heif_chroma_420, &image);
    (void)err;

    heif_image_add_plane(image, heif_channel_Y, cinfo.output_width,
                         cinfo.output_height, 8);
    heif_image_add_plane(image, heif_channel_Cb, (cinfo.output_width + 1) / 2,
                         (cinfo.output_height + 1) / 2, 8);
    heif_image_add_plane(image, heif_channel_Cr, (cinfo.output_width + 1) / 2,
                         (cinfo.output_height + 1) / 2, 8);
  ...
  return std::shared_ptr<heif_image>(
      image, [](heif_image* img) { heif_image_release(img); });
}
#else
std::shared_ptr<heif_image> loadJPEG(const char* filename) {
...
std::shared_ptr<heif_image> loadPNG(const char* filename) {
  ...
  if (png_get_valid(png_ptr, info_ptr, PNG_INFO_iCCP)) {
    if (PNG_INFO_iCCP == png_get_iCCP(png_ptr, info_ptr, &name,
                                      &compression_type, &png_profile_data,
                                      &profile_length)) {
 ...
 */
  ...
  if (band == 1) {
    err = heif_image_create((int)width, (int)height, heif_colorspace_YCbCr,
                            heif_chroma_monochrome, &image);
    (void)err;
    ...
    if (has_alpha) {
      heif_image_add_plane(image, heif_channel_Alpha, (int)width, (int)height,
                           8);
    ...
    heif_image_add_plane(image, heif_channel_interleaved, (int)width,
                         (int)height, has_alpha ? 32 : 24);
  ...
  if (color_profile_valid && profile_length > 0) {
    heif_image_set_raw_color_profile(image, "prof", profile_data,
...
void list_encoder_parameters(heif_encoder* encoder) {
  ...
  for (int i = 0; params[i]; i++) {
    ...
    case heif_encoder_parameter_type_integer: {
      int value;
      heif_error error =
          heif_encoder_get_parameter_integer(encoder, name, &value);
      (void)error;

      std::cerr << "  " << name << ", default=" << value;

      int have_minmax, minimum, maximum;
      error = heif_encoder_parameter_integer_valid_range(
          encoder, name, &have_minmax, &minimum, &maximum);
    ...
    case heif_encoder_parameter_type_boolean: {
      int value;
      heif_error error =
          heif_encoder_get_parameter_boolean(encoder, name, &value);
      (void)error;
    ...
    case heif_encoder_parameter_type_string: {
      ...
      heif_error error =
          heif_encoder_get_parameter_string(encoder, name, value, value_size);
      (void)error;

      std::cerr << "  " << name << ", default=" << value;

      const char* const* valid_options;
      error = heif_encoder_parameter_string_valid_values(encoder, name,
                                                         &valid_options);
...
void set_params(struct heif_encoder* encoder, std::vector<std::string> params) {
  for (std::string p : params) {
    ...
    struct heif_error error =
        heif_encoder_set_parameter(encoder, name.c_str(), value.c_str());
    if (error.code) {
...
int main(int argc, char** argv) {
  ...
  while (true) {
    int option_index = 0;
    int c =
        getopt_long(argc, argv, "hq:Lo:vPp:t:", long_options, &option_index);
    if (c == -1)
      break;
...
#define MAX_ENCODERS 5
  ...
  int count = heif_context_get_encoder_descriptors(
      context.get(), heif_compression_HEVC, nullptr, encoder_descriptors,
      MAX_ENCODERS);
  ...
  if (count > 0) {
    ...
    heif_error error = heif_context_get_encoder(
        context.get(), encoder_descriptors[0], &encoder);
    if (error.code) {
  ...
  for (; optind < argc; optind++) {
    ...
    if (heif_image_get_colorspace(image.get()) == heif_colorspace_RGB &&
        lossless) {
      std::cerr << "Warning: input image is in RGB colorspace, but encoding is "
                   "currently\n"
                << "  always done in YCbCr colorspace. Hence, even though you "
                   "specified lossless\n"
                << "  compression, there will be differences because of the "
                   "color conversion.\n";
    }
    ...
    error = heif_context_encode_image(context.get(), image.get(), encoder,
                                      options, &handle);
    if (error.code != 0) {
    ...
    if (thumbnail_bbox_size > 0) {
      ...
      error = heif_context_encode_thumbnail(
          context.get(), image.get(), handle, encoder, options,
          thumbnail_bbox_size, &thumbnail_handle);
      if (error.code) {
